...
#include "nav2_constrained_smoother_old/constrained_smoother.hpp"
...
void ConstrainedSmootherOld::configure(
  ...
  std::shared_ptr<nav2_costmap_2d::FootprintSubscriber>)
{
  ...
  smoother_ = std::make_unique<nav2_constrained_smoother_old::Smoother>();
  optimizer_params_.get(node.get(), name);
  smoother_params_.get(node.get(), name);
  smoother_->initialize(optimizer_params_);
}

void ConstrainedSmootherOld::cleanup()
{
  RCLCPP_INFO(
    logger_,
    "Cleaning up smoother: %s of type"
    " nav2_constrained_smoother_old::ConstrainedSmootherOld",
    plugin_name_.c_str());
}

void ConstrainedSmootherOld::activate()
{
  RCLCPP_INFO(
    logger_,
...
void ConstrainedSmootherOld::deactivate()
{
  RCLCPP_INFO(
    logger_,
...
bool ConstrainedSmootherOld::smooth(nav_msgs::msg::Path & path, const rclcpp::Duration & max_time)
{
