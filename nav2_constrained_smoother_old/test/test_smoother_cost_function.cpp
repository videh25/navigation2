...
class TestableSmootherCostFunction : nav2_constrained_smoother_old::SmootherCostFunction
{
public:
  TestableSmootherCostFunction(
    ...
    const nav2_constrained_smoother_old::SmootherParams & params,
...
TEST_F(Test, testingCurvatureResidual)
{
  ...
  TestableSmootherCostFunction fn(
    ...
    nav2_constrained_smoother_old::SmootherParams(), 0.0
  );
  ...
  nav2_constrained_smoother_old::SmootherParams params_no_min_turning_radius;
...
TEST_F(Test, testingUtils)
{
  ...
  auto center = nav2_constrained_smoother_old::arcCenter(pt_prev, pt, pt_next, false);
  ...
  auto tangent =
    nav2_constrained_smoother_old::tangentDir(pt_prev, pt, pt_next, false).normalized();
  EXPECT_NEAR(tangent[0], 0, 1e-10);
  EXPECT_NEAR(std::abs(tangent[1]), 1, 1e-10);

  // no rotation when mid point is a cusp
  tangent = nav2_constrained_smoother_old::tangentDir(pt_prev, pt, pt_next, true).normalized();
